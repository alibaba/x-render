...
- **addons.setValueByPath**: 注意挂在 addons 下面。用于在本组件内修改其他组件的值 setValueByPath(path, value)
- **addons.getValueByPath**: 用于通过路径字符串获取值 getValue(path)。例如 `getValueByPath('a.b[2].c')`。getValue() 获取 formData
