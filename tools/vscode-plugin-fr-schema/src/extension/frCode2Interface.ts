...
import { getInterfaceFromModel, getModelFromSchema, getSchemaFromCode } from './utils';

export class frCode2InterfaceProvider {
  public static register(): vscode.Disposable {
    return vscode.commands.registerCommand(frCode2InterfaceProvider.viewType, () => {
      const { document, selection } = vscode.window.activeTextEditor || {};
      if (document && selection) frCode2InterfaceProvider.generate(document, selection);
    });
  }

  private static readonly viewType = 'frSchema.c2i';

  public static async generate(document: vscode.TextDocument, selection: vscode.Selection) {
    try {
