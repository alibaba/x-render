...
export function flatProperties(properties, ignoreObject = false, preKey = '') {
  ...
  Object.keys(properties).forEach(key => {
    ...
    if (get(properties[key], 'type', '').toLocaleLowerCase() === 'object') {
      ...
      obj = { ...obj, ...flatProperties(properties[key].properties, ignoreObject, preKey ? `${preKey}[${key}]` : `[${key}]`) }
    }
  })
  return obj;
}
...
export function enumToEnumNames(data, enums, enumNames = enums) {
  ...
  if (Array.isArray(data)) {
    ...
    retData = data.map(item => (enumNames[enums.findIndex(i => String(i) === String(item))]) || enumNames.findIndex(i => String(i) === String(item))).join('_');
  } else {
    // 原有的 data 值是 'pear' 这种，换成 梨子
    retData = enumNames[enums.findIndex(i => String(i) === String(data))] || enumNames.findIndex(i => String(i) === String(data));
  }
  return retData || data;
}
...
export function enumNamesToEnum(data, enums, enumNames = enums) {
  let retData = null;
  if (Array.isArray(data)) {
    retData = data.map(item => enums[enumNames.findIndex(i => String(i) === String(item))] || enums.find(i => String(i) === String(item)));
  } else {
    retData = enums[enumNames.findIndex(i => String(i) === String(data))] || enums.find(i => String(i) === String(data));
  }
  return retData || data;
}
...
export function generateSheetHeader(properties) {
  ...
  return Object.keys(flatProp).map(key => {
    ...
    return iProp.type === 'array' && iProp?.items?.type === 'object' ? `【此列勿填】${iProp.title || key}` : `${iProp.title || key}`
  })
}

export function generateSheetData(properties, arrayData) {
  function flatObject(obj) {
    return Object.values(obj).map(objVal => isObject(objVal) ? flatObject(objVal) : objVal)
  }
  // 把数组内的对象降维下来
