...
var getValidateText = function getValidateText() {
  ...
  if (type === 'string') {
    ...
    if (_finalValue && (minLength || minLength === 0)) {
      if (!_finalValue || !(0, _isLength.default)(_finalValue, parseInt(minLength, 10), undefined)) {
    ...
    if (format === 'url') {
      if (usePattern) {} else if (finalValue && !(0, _utils.isUrl)(finalValue)) {
        return message && message.url || '请输入正确的url格式';
      }
    }

    if (format === 'email') {
      if (usePattern) {} else if (finalValue && !(0, _utils.isEmail)(finalValue)) {
