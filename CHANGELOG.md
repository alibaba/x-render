...
注 2：在 0.3 及以前的版本，form-render 使用了固定长度的标签、也未遵照 antd 等常见表单的标签展示原则。在 0.4 版本我们在样式上统一到类似 antd 的规范标准，同时标签长度也可以通过全局的 labelWidth 参数控制

- [+] 新增 labelWidth 参数，使用了类似于 antd 的 1 - 12 作为长度标准，默认长度 3（即 25%）
